...
@section('breadcrumb')
    <a href="{{ route('donations.create') }}" class="btn btn-primary btn-sm">Create</a>
@endsection
@section('content')
                                    ...
                                    @if (count($data) > 0)
                                        @foreach ($data as $row)
                                            <tr class="id{{ $row->id }}">
                                                ...
                                                <td class="text-left">
                                                    <a href="{{ route('donations.edit', $row->id) }}">Edit</a>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function() {
            $('.btn-delete').on('click', function(e) {
                ...
                var id = str.slice(1);
                var url = '{{ route('donations.destroy', ':id') }}';
